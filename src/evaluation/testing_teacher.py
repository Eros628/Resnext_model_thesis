...
# CONFIG
# ----------------------------
MODEL_PATH = r"D:\Thesis traning\models\resnext50_lfcc_v1_asvspoof2019_400.pth"
...
# ----------------------------
# LFCC extraction (training version)
# ----------------------------
...
def load_audio_and_extract_lfcc(path):
    ...
    lfcc = extract_lfcc_from_waveform(data)
    ...
    return lfcc.T[np.newaxis, :, :].astype(np.float32)
...
# ----------------------------
def main():
    print("Loading ASVspoof2021 eval protocol...")
    test_files, test_labels = load_asvspoof2021_protocol(PROTO_PATH, AUDIO_DIR, max_samples=MAX_SAMPLES)
    print(f"Loaded {len(test_files)} samples.")

    dataset = LFCCDataset(test_files, test_labels)
    loader = DataLoader(dataset, batch_size=8, shuffle=False, num_workers=0)

    print("Loading model...")
    model = ResNeXt50LFCC(num_classes=2, pretrained=False).to(DEVICE)
